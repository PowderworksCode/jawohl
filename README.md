...
Python and JavaScript bindings are generated by [jedem][] from one annotated
Rust surface — see [`bindings/`](bindings/), which builds and runs both against
real host processes.
...
**They cover the batch-shaped subset, not the streaming API.** jedem v1 exposes
functions over plain values, and jawohl's `Stream` is a stateful handle, so each
call re-parses from the beginning. That is fine for a tool call and wrong for a
long document; the incremental API reaches those languages when jedem grows
handles.

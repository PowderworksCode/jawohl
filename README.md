...
Python and JavaScript bindings are generated by [jedem][] from **jawohl's own
source** — `Stream`, `complete_json`, `Syntax` and `Validation` carry their
annotations where they are defined, and nothing restates the API anywhere else.
See [`bindings/`](bindings/), which builds and runs both against real host
processes.

`Stream` crosses as a handle, so these get the incremental parser itself, not a
batch shim that re-parses from byte zero on every call:
...
Four methods are not across yet — `snapshot` and `changes` need unions,
`error` and `lowering_report` need records. `changes` is the one that matters:
until it lands, you poll `status` instead of consuming events. `bindings/`
names all four.

Said plainly because the 0.1 README promised JavaScript and Python wrappers
"soon" and never delivered: what exists is in `bindings/`, and what does not is
the paragraph above.
